...
class DebugDumper:
    ...
    def dump_session(self, session_id: str, session_data: Dict[str, Any], reason: str = "manual", include_voxels: bool = False) -> str:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = self.dump_dir / f"dump_{session_id}_{timestamp}_{reason}.json"
    ...
    def dump_generation_error(
        ...
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = self.dump_dir / f"ERROR_{session_id}_{timestamp}.json"
    ...
    def dump_comparison(self, session_id: str, before: Dict[str, Any], after: Dict[str, Any], action: str) -> str:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filepath = self.dump_dir / f"compare_{session_id}_{timestamp}.json"
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump({"metadata": {"session_id": session_id, "timestamp": timestamp, "type": "COMPARISON", "action": action}, "before": before, "after": after}, f, indent=2, default=str)
        return str(filepath)
